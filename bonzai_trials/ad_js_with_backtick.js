...
function loadAd(container,reqJson){
	...
	//adJSContent += `<scr`;
	adJSContent += `<script type="text/javascript">
	    //alert(12); 
		var isNonIframeAd = true; 
		console.log(isNonIframeAd); 
		if(!window.adContainer){
			window.adContainer = "mizu-ad";//window.frameElement.getAttribute("adContainer")
		}; 
...
function isAdPixelPreview() {
    return false;//bonzai.plugin.previewEnv().isAdPixelPreview();
}
...
})(isNonIframeAd,window.adContainer);function adload(){alert(56789);}<\/script>`;
//adJSContent += `ipt>`;
	//var adVariables = `<scr`;
    var adVariables = `<script type="text/javascript">window.adContainer = window.frameElement.getAttribute("adContainer"); window.requestJson = window.frameElement.getAttribute("reqJson");</script>`;
    //adVariables += `ipt>`;
...
console.log("non iframe 1");
		...
		nonIFrame.appendChild(handleScriptAddition(unescape((adVariables))));
		nonIFrame.appendChild(handleScriptAddition(unescape((adJSContent))));
		console.log("non iframe 6");
		setTimeout(function(){test();},500);
		//	setTimeout(function(){onAdLoad();},500);
		// window[window.adContainer].;
